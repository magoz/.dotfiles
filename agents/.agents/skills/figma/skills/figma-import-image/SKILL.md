...
> **Deprecated fallback:** Current Figma guidance requires `upload_assets` for every external image, including URLs, local files, generated bytes, and screenshots. Do not use this workflow unless `upload_assets` is demonstrably unavailable and the user accepts a legacy workaround.

Get a bitmap image into a Figma file as a real frame with an `Image` hash. This older workflow works around the heavy restrictions in the `use_figma` JS sandbox.

## When to use

Use only when all of these are true:

- `upload_assets` is unavailable or has failed for a documented reason
- The user accepts this unsupported legacy workaround
- A mockup, photo, render, screenshot, or artwork reference still needs to live in a Figma file
- The image is at a public URL or a local file that can be hosted first
- The target Figma file already exists (use `create_new_file` first if not)

## Why this skill exists

Before `upload_assets` existed, `use_figma` ran in a restricted JS sandbox and the older capture flow was the only reliable workaround found locally. Keep this workflow only for exceptional fallback use. Do not retry the blocked Plugin API paths documented in [references/sandbox-limits.md](./references/sandbox-limits.md); use `upload_assets` first.
