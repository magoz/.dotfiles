...
LAUNCHER="$REPO_ROOT/home/scripts/.local/bin/aha"
TEST_ROOT=$(mktemp -d)
...
rg -q 'missing .*aha/dist/cli/aha.js' "$TEST_ROOT/output"
...
mkdir -p "$TEST_HOME/aha/dist/cli"
cat >"$TEST_HOME/aha/dist/cli/aha.js" <<'JS'
const assert = require('node:assert/strict')
const fs = require('node:fs')
assert.equal(process.cwd(), process.env.TEST_CALLER_DIR)
assert.equal(process.env.AHA_ENDPOINT ?? '', process.env.TEST_EXPECTED_ENDPOINT ?? '')
...
cd "$TEST_CALLER_DIR"
expect_exit 23 env HOME="$TEST_HOME" AHA_ENDPOINT= "$LAUNCHER" upload 'page with spaces.html' \
  --endpoint http://127.0.0.1:3939
printf 'ok: launcher preserves cwd, relative paths, argument boundaries, and exit status\n'

mkdir -p "$TEST_HOME/.config/aha"
printf '%s' 'https://aha-origin.example' >"$TEST_HOME/.config/aha/endpoint"
expect_exit 23 env HOME="$TEST_HOME" AHA_ENDPOINT= TEST_EXPECTED_ENDPOINT=https://aha-origin.example \
  "$LAUNCHER" upload 'page with spaces.html' --endpoint http://127.0.0.1:3939
expect_exit 23 env HOME="$TEST_HOME" AHA_ENDPOINT=https://override.example \
...
printf '%s\n' "$literal" >"$TEST_HOME/.config/aha/endpoint"
expect_exit 23 env HOME="$TEST_HOME" AHA_ENDPOINT= TEST_EXPECTED_ENDPOINT="$literal" \
  "$LAUNCHER" upload 'page with spaces.html' --endpoint http://127.0.0.1:3939
[[ ! -e "$TEST_CALLER_DIR/SHOULD_NOT_EXIST" ]]
printf 'ok: endpoint file is read as data, never evaluated\n'

: >"$TEST_HOME/.config/aha/endpoint"
expect_exit 1 env HOME="$TEST_HOME" AHA_ENDPOINT= "$LAUNCHER" upload 'page with spaces.html'
